...
export default {
  load() {
    ...
    return {
      setTrainningDataset (datasets) {
        [trainningData, normalizationData] = convertToTensor(datasets);  
      },
    
      async train(trainningParameter = { batchSize: 32, epochs: 50 }) {
        model.compile({
        ...
        const { inputs, labels } = trainningData
